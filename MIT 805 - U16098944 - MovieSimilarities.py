...
from itertools import combinations # needed for the movie pair permutations

class Recommendations(MRJob):

    def configure_args(self):
        super(Recommendations, self).configure_args()
        self.add_file_arg('--items', help='Path to u.item') #look up file for movies to be on every node

    def load_movie_names(self): #loads u.item file - then maps movie ID to movie names using dictionary
    ...
    def mapper_parse_input(self, key, line):
        # Outputs userID => (movieID, rating) step 1 - user ID is the key
